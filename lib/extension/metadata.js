...
//  LOGGER.debug('Peer [%s] notified of metadata message.', payload);

  var decodedPayload = bencode.decode(payload),
      messageDetail = decodedPayload,
      messageType = messageDetail['msg_type'],
      activePieces = this._activePieces,
      requestedPieces = this._activePeers[peer.getIdentifier()];

  LOGGER.debug(decodedPayload);
