...
Peer.prototype.requestPiece = function(piece) {
  ...
  if (piece && !piece.isComplete()) {
    if (!self.pieces[piece.index]) {
      ...
      piece.once(Piece.COMPLETE, function(obj) {
    ...
    while (self.numRequests < MAX_REQUESTS && (nextChunk = piece.nextChunk())) {
      ...
      LOGGER.debug("requests for piece %d offset %d", piece.index, nextChunk.begin);
...
Peer.prototype.setTorrent = function(torrent) {
  ...
  LOGGER.debug("Adding torrent %s to  %s", torrent._metadata.name, self.getIdentifier());
  this.bitfield = new BitField(torrent.bitfield ? torrent.bitfield.length : 0);
  LOGGER.debug("BitField size %s", this.bitfield.length);
  if (this.stream) {
    if (this.initialised) {
...
Peer.prototype.isReady = function() {
  LOGGER.debug("isReady : amInterested %d, chocked %d, numREquest %d", this.amInterested, this.choked, this.numRequests);
            ...
            LOGGER.debug('Peer [%s] sent keep alive', self.getIdentifier());
              ...
              switch (message.code) {
                ...
                case Message.BITFIELD:
                  ...
                  LOGGER.debug(self.bitfield.length);
                  self.emit(Peer.UPDATED);
                  break;
