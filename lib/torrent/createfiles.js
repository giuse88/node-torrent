...
function createFiles(downloadPath, metadata, callback) {
  ...
  var basePath = path.join(downloadPath, metadata.name.toString());

  LOGGER.debug( "File size : " + metadata.length);
  LOGGER.debug( "File path : " + basePath );
...
function nextFile(basePath, files, processedFiles, offset, callback) {
  if ( files.lenght || files.length === 0) {
