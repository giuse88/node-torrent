...
function Torrent(clientId, clientPort, downloadPath, dataUrl, extensions) {
  ...
  TorrentData.load(dataUrl, function(error, metadata, trackers) {
    LOGGER.debug('Load torrent : ' + dataUrl);
    if (error) {
      ...
      trackers.forEach(function(tracker) {
        ...
        LOGGER.debug("Added tracker  %s", tracker.url.href);
      });
    torrent._initialise();
    }
  });
 ...
 */
    ...
    peer.on(Peer.UPDATED, function() {
      LOGGER.debug("BIT" + torrent.bitfield);
...
Torrent.prototype._initialise = function() {
  ...
  /*
  torrent._pieces = torrent._metadata.pieces;
  LOGGER.debug('Initialized torrent pieces : %s', torrent.pieces);
  torrent.bitfield = new BitField(torrent._pieces.length);
  LOGGER.debug('Initialized bitField of size %d', torrent._pieces.length);
  torrent._setStatus(Torrent.READY);
  */


  LOGGER.debug('Metadata is complete, initialising files and pieces.');
  LOGGER.debug('Dowloand Path : ' + this._downloadPath + " "  + this._metadata);
  //LOGGER.debug(this._metadata);
  ...
  createFiles(this._downloadPath, this._metadata, function(error, _files, _size) {
      if (error) {
        ...
        createPieces(torrent, torrent._metadata.pieces, _files, torrent._metadata['piece length'],
            _size, function(error, _pieces) {
          if (error) {
            torrent._setStatus(Torrent.ERROR, error);
          } else {
            LOGGER.debug(torrent._pieces);
