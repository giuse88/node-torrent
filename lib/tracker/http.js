...
HTTP.prototype = {
  ...
  _makeRequest: function() {
    ...
    LOGGER.debug( "Url request : " + options.host + options.path + options.port);
    var req = http.get(options, function(res) {
      ...
      res.on('data', function(chunk) {
        buffers.push(chunk);
        length += chunk.length;
        console.log("Length chuck : " + chunk.length);
      });
      res.on('end', function() {
        var body = new Buffer(length);
        console.log("Body size : " + length);
      /*  var pos = 0;
        //TODO Test better the merge function 
        LOGGER.debug("Body received : " + body.toString() );
        for (var i = 0; i < buffers.length; i++) {
          body.write(buffers[i], pos);
          pos += buffers[i].length;
        }
       */ 
        
        if (res.statusCode === 200) {
          var response = bencode.decode(Buffer.concat(buffers));
    ...
    if (response['failure reason']) {
      ...
      if (response.peers) {
		//		if (typeof(response.peers) === 'string') {
					var peers = response.peers; //new Buffer(response.peers, 'binary');
