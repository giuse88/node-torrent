...
var Tracker = function(urls) {
  ...
  if (!Array.isArray(urls)) {
    this._urls = new Array(urls);
  } else {
    this._urls = urls;
  }
  console.log(this._utlrs[0].toString());
  i*/
  // TODO: need to step through URLs as part of announce process
  this.url = require('url').parse(urls.toString());
  this.torrent = null;
  this.state = STOPPED;
  this.seeders = 0;
  this.leechers = 0;
...
Tracker.prototype._announce = function(event) {
  ...
  LOGGER.debug('Announce' + (event ? ' ' + event : '') + " Protocol : " + this.url.protocol);
  ...
  if (handlerClass) {
    ...
    var data = {
      ...
      left: this.torrent._metadata.length - this.torrent.stats.downloaded, 
      downloaded : this.torrent.stats.downloaded, 
      uploaded : this.torrent.stats.uploaded
    };
